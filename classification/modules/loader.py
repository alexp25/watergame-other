...
def format_data(df):
    ...
    d = {'Multi-Family': 1, 'Irrigation - Multi-Family': 2, "Residential": 3, "Irrigation - Residential": 4}
    df['Customer_Class'] = df['Customer_Class'].map(d)
    return df

def get_dataset_xy(df):
    # separate the feature (input) columns from the target column
    df = df[['Code_Year_Month', 'Postal_Code', 'Total_Gallons', 'Customer_Class']]
    df = df.fillna(0)

    features = list(df.columns)
    n_features = len(features)
    target_features = features[n_features-1]
